...
public class Local3TaxaTransport {
    ...
    public Local3TaxaTransport(
        ...
        this.approximateFelsenstein = gridData.approximateFelsenstein;
    ...
    private static GridData buildGridData(
        ...
        double maxLogLikelihood = populateLogLikelihoodGrid(
                approximateFelsenstein,
                grid,
                distanceOffsets,
    ...
    private static void validateConditioningShape(
        ...
        for (double distance : conditioningDistances) {
            if (!Double.isFinite(distance)) {
                throw new IllegalArgumentException("conditioning distances must be finite");
            }
        }
    }
