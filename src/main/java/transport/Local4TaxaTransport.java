...
public class Local4TaxaTransport {
    ...
    private final double[][][] cdf2GivenD0D1Grid;
    private final double[][] cdf1GivenD0Grid;
    private final double[] cdf0;
    private final double maxLogLikelihood;
    private final double totalMass;

    public Local4TaxaTransport(List<String> taxonIds, List<Double> taxonHeights, Alignment alignment, SiteModelInterface siteModel, RealScalarParam<?> clockRate, double maxDistance) {
        if (!Double.isFinite(maxDistance) || maxDistance <= 0.0) {
            throw new IllegalArgumentException("maxDistance must be finite and positive");
        }

        GridData gridData = buildGridData(taxonIds, taxonHeights, alignment, clockRate, maxDistance);
        ...
        this.cdf2GivenD0D1Grid = gridData.cdf2GivenD0D1Grid;
    ...
    public double[] transport(double[] distances) {
        ...
        transported[2] = this.invGaussianCDF(this.felsensteinCDF(2, shiftedDistances));
    ...
    public double[] transportBack(double[] transportedState) {
        ...
        distances[2] = this.inverseCDF2(distances[0], distances[1], this.gaussianCDF(transportedState[2]));
    ...
    private double felsensteinCDF(int i, double[] distances) {
        ...
        if (i == 0) {
            ...
            return this.evaluateCDF2(distances[0], distances[1], distances[2]);
        }

        throw new IllegalArgumentException("only three distance dimensions are supported");
    }
    ...
    public double felsensteinLogPDF(double[] distances) {
        ...
        double clamped2 = this.clampDistance(shiftedDistances[2]);
        return this.trilinearInterpolateLogLikelihood(clamped0, clamped1, clamped2);
    }
    ...
    private static GridData buildGridData(
        ...
        Approximate4TaxaFelsenstein approximateFelsenstein = new Approximate4TaxaFelsenstein(taxonIds, alignment, clockRate);
        ...
        double[][][] logLikelihoodGrid = new double[GRID_SIZE][GRID_SIZE][GRID_SIZE];
        ...
        double[][][] cdf2GivenD0D1Grid = buildConditionalCDF2Grid(offsetWeightGrid, grid[0], gridSpacing);
        double[][] marginal1 = buildMarginal1(cdf2GivenD0D1Grid);
        double[][] cdf1GivenD0Grid = buildConditionalCDF1Grid(marginal1, grid[0], gridSpacing);
    ...
    private static double[] buildDistanceOffsets(List<Double> taxonHeights) {
        if (taxonHeights.size() != 4) {
            throw new IllegalArgumentException("four-taxon transport requires exactly four taxon heights");
        }
    ...
    private static double populateLogLikelihoodGrid(
            Approximate4TaxaFelsenstein approximateFelsenstein,
            double[] grid,
    ...
    private static double[][][] buildOffsetWeightGrid(double[][][] logLikelihoodGrid, double maxLogLikelihood) {
        double[][][] weights = new double[GRID_SIZE][GRID_SIZE][GRID_SIZE];

        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                for (int k = 0; k < GRID_SIZE; k++) {
                    double logLikelihood = logLikelihoodGrid[i][j][k];
                    weights[i][j][k] = Double.isFinite(logLikelihood)
    ...
    private static double[][][] buildConditionalCDF2Grid(
            double[][][] offsetWeightGrid,
            double firstGridDistance,
            double gridSpacing
    ) {
        double[][][] cdf = new double[GRID_SIZE][GRID_SIZE][GRID_SIZE];

        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                cdf[i][j][0] = trapezoidArea(0.0, offsetWeightGrid[i][j][0], firstGridDistance);
                for (int k = 1; k < GRID_SIZE; k++) {
                    cdf[i][j][k] = cdf[i][j][k - 1]
                            + trapezoidArea(offsetWeightGrid[i][j][k - 1], offsetWeightGrid[i][j][k], gridSpacing);
                }
            }
        }
    ...
    private static double[][] buildMarginal1(double[][][] cdf2GivenD0D1Grid) {
        double[][] marginal = new double[GRID_SIZE][GRID_SIZE];
        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                marginal[i][j] = cdf2GivenD0D1Grid[i][j][LAST_GRID_INDEX];
            }
        }
        return marginal;
    }
    ...
    private static double[][] buildConditionalCDF1Grid(
        ...
        for (int i = 0; i < GRID_SIZE; i++) {
            cdf[i][0] = trapezoidArea(0.0, marginal1[i][0], firstGridDistance);
            for (int j = 1; j < GRID_SIZE; j++) {
                cdf[i][j] = cdf[i][j - 1] + trapezoidArea(marginal1[i][j - 1], marginal1[i][j], gridSpacing);
            }
        }
    ...
    private double evaluateCDF2(double distance0, double distance1, double distance2) {
        double clamped0 = this.clampDistance(distance0);
        double clamped1 = this.clampDistance(distance1);
        double clamped2 = this.clampDistance(distance2);
        GridPosition position0 = this.getGridPosition(clamped0);
        GridPosition position1 = this.getGridPosition(clamped1);

        double numerator = this.bilinearInterpolate(
                this.linearInterpolate(this.grid, this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.lowerIndex], clamped2),
                this.linearInterpolate(this.grid, this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.upperIndex], clamped2),
                this.linearInterpolate(this.grid, this.cdf2GivenD0D1Grid[position0.upperIndex][position1.lowerIndex], clamped2),
                this.linearInterpolate(this.grid, this.cdf2GivenD0D1Grid[position0.upperIndex][position1.upperIndex], clamped2),
                position0.fraction,
                position1.fraction
        );

        double denominator = this.bilinearInterpolate(
                this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.lowerIndex][LAST_GRID_INDEX],
                this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.upperIndex][LAST_GRID_INDEX],
                this.cdf2GivenD0D1Grid[position0.upperIndex][position1.lowerIndex][LAST_GRID_INDEX],
                this.cdf2GivenD0D1Grid[position0.upperIndex][position1.upperIndex][LAST_GRID_INDEX],
                position0.fraction,
                position1.fraction
        );
    ...
    private double inverseCDF2(double distance0, double distance1, double probability) {
        double clamped0 = this.clampDistance(distance0);
        double clamped1 = this.clampDistance(distance1);
        GridPosition position0 = this.getGridPosition(clamped0);
        GridPosition position1 = this.getGridPosition(clamped1);
        ...
        for (int k = 0; k < GRID_SIZE; k++) {
            conditionalCDF[k] = this.bilinearInterpolate(
                    this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.lowerIndex][k],
                    this.cdf2GivenD0D1Grid[position0.lowerIndex][position1.upperIndex][k],
                    this.cdf2GivenD0D1Grid[position0.upperIndex][position1.lowerIndex][k],
                    this.cdf2GivenD0D1Grid[position0.upperIndex][position1.upperIndex][k],
                    position0.fraction,
                    position1.fraction
            );
        }
    ...
    private double trilinearInterpolateLogLikelihood(double x, double y, double z) {
        GridPosition xPosition = this.getGridPosition(x);
        GridPosition yPosition = this.getGridPosition(y);
        GridPosition zPosition = this.getGridPosition(z);

        return this.trilinearInterpolate(
                this.getFiniteLogLikelihood(xPosition.lowerIndex, yPosition.lowerIndex, zPosition.lowerIndex),
                this.getFiniteLogLikelihood(xPosition.lowerIndex, yPosition.lowerIndex, zPosition.upperIndex),
                this.getFiniteLogLikelihood(xPosition.lowerIndex, yPosition.upperIndex, zPosition.lowerIndex),
                this.getFiniteLogLikelihood(xPosition.lowerIndex, yPosition.upperIndex, zPosition.upperIndex),
                this.getFiniteLogLikelihood(xPosition.upperIndex, yPosition.lowerIndex, zPosition.lowerIndex),
                this.getFiniteLogLikelihood(xPosition.upperIndex, yPosition.lowerIndex, zPosition.upperIndex),
                this.getFiniteLogLikelihood(xPosition.upperIndex, yPosition.upperIndex, zPosition.lowerIndex),
                this.getFiniteLogLikelihood(xPosition.upperIndex, yPosition.upperIndex, zPosition.upperIndex),
                xPosition.fraction,
                yPosition.fraction,
                zPosition.fraction
        );
    ...
    private double getFiniteLogLikelihood(int i, int j, int k) {
        double logLikelihood = this.logLikelihoodGrid[i][j][k];
        if (Double.isFinite(logLikelihood)) {
    ...
    private double trilinearInterpolate(
        ...
        double lowerX = this.bilinearInterpolate(c000, c001, c010, c011, yFraction, zFraction);
        double upperX = this.bilinearInterpolate(c100, c101, c110, c111, yFraction, zFraction);
        return this.interpolate(lowerX, upperX, xFraction);
    }

    private double bilinearInterpolate(
            double lowerXLowerY,
            double lowerXUpperY,
            double upperXLowerY,
            double upperXUpperY,
        ...
        double lowerY = this.interpolate(lowerXLowerY, upperXLowerY, xFraction);
        double upperY = this.interpolate(lowerXUpperY, upperXUpperY, xFraction);
        return this.interpolate(lowerY, upperY, yFraction);
    }
    ...
    private void validateDistanceDimension(double[] distances) {
        if (distances.length != 3) {
            throw new IllegalArgumentException("grid transport requires exactly three distance dimensions");
        }
    }
