 ...
 */
public class LargeJumpMALAOperator extends SliceOperator {
    ...
    public double proposal(Supplier<Double> computeCurrentLogLikelihood, State state) {
        ...
        double[] jumpedState = add(oldStateJump, jump);
        ...
        this.applyVector(this.optimizationAdapters, proposedState, nodeId);
        ...
        double ratio = this.logDensity(oldStateOpt, reverseOptimizedState) - this.logDensity(proposedState, optimizedState);
    ...
    private double[] optimizeState(int nodeId, Supplier<Double> computeCurrentLogLikelihood) {
        double[] best = this.getMutableVector(this.optimizationAdapters, this.numOptimizationMutable, nodeId);
        double bestLogLikelihood = computeCurrentLogLikelihood.get();

        for (int step = 0; step < this.numOptimizationSteps; step++) {
            double[] noise = this.optimizationCovarianceSampler.sampleConditionally(new double[] {}, 0.1 * this.noiseScale);
            double[] candidate = add(best, noise);
            this.applyVector(this.optimizationAdapters, candidate, this.numOptimizationMutable);
